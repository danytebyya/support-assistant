(function (global) {
  ...
  function init(config) {
    ...
    const unlockBodyScroll = () => {
      ...
      document.documentElement.style.overflow = savedBodyStyle.htmlOverflow || "";
      document.body.style.backgroundColor = savedBodyStyle.bodyBg || "";
    ...
    document.addEventListener("touchmove", (e) => {
      if (state.ui?.panel?.classList.contains("open") && window.innerWidth <= 768) {
        ...
        if (messages && (messages === e.target || messages.contains(e.target) || path.includes(messages))) {
          return;
        }
        e.preventDefault();
      }
    ...
    const toggle = () => {
      ...
      if (open) {
        ...
        state.ui.panel.style.removeProperty("--chat-height");
        scrollToBottom();
      }
    };
