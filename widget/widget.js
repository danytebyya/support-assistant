(function (global) {
  ...
  const css = `
    ...
    *{box-sizing:border-box}.fab{position:fixed;right:22px;bottom:22px;width:60px;height:60px;border:0;border-radius:var(--button-radius);background:var(--lime);color:#fff;cursor:pointer;display:grid;place-items:center;opacity:1;transform:scale(1);transition:background-color .2s,opacity .16s ease,transform .24s ease;z-index:2147483647}.fab:hover{background:var(--lime-hover)}svg{width:24px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.fab img{width:24px;height:24px}.send img{width:19px;height:19px}.header-action img{width:16px;height:16px;filter:brightness(0) invert(1)}.header-action img,.header-action svg{transition:transform .14s ease}.header-action:active img,.header-action:active svg{transform:scale(.78)}.logo img{width:24px;height:24px}img{display:block;object-fit:contain}
    ...
    header{background:var(--lime);color:#fff;padding:19px 20px;display:flex;align-items:center;gap:8px}.logo{width:38px;height:38px;flex:0 0 38px;margin-right:4px;background:var(--lime);color:var(--dark);display:grid;place-items:center;font-weight:900}.logo img{width:38px;height:38px;border-radius:0;object-fit:contain}.title{flex:1;display:flex;flex-direction:column;gap:1px}.title b{display:block;font-size:15px;line-height:1.15}.title span{font-size:12px;line-height:1.15;color:#fff;opacity:.82}.header-action{position:relative;width:32px;height:32px;flex:0 0 32px;border:0;color:#fff;background:transparent;cursor:pointer;padding:7px;border-radius:var(--button-radius);display:grid;place-items:center;transition:background-color .18s}.header-action:hover{background:#ffffff24}.header-action svg{width:16px;height:16px}.header-action:after{content:attr(data-tooltip);position:absolute;top:calc(100% + 9px);right:0;width:max-content;max-width:130px;padding:5px 8px;border:0;border-radius:.45rem;background:#fff;color:#111;font:500 11px/1.25 Inter,system-ui,-apple-system,sans-serif;white-space:nowrap;box-shadow:0 4px 12px #1020182e;opacity:0;visibility:hidden;pointer-events:none;transform:translateY(-2px);transition:opacity .12s ease,transform .12s ease,visibility 0s linear .12s;z-index:2}.header-action:hover:after,.header-action:focus-visible:after{opacity:1;visibility:visible;transform:translateY(0);transition-delay:1s}.header-action:focus-visible{outline:2px solid #fff;outline-offset:2px}
    ...
    form{padding:13px;background:var(--surface);border-top:var(--border);display:flex;gap:9px;align-items:flex-end}textarea{font:inherit;resize:none;height:43px;min-height:43px;max-height:calc(4.35em + 24px);overflow-y:hidden;flex:1;border:var(--border);border-radius:var(--button-radius);padding:11px 12px;outline:0}textarea:focus{border-color:#c9cdcb}.send{width:43px;height:43px;border:0;border-radius:var(--button-radius);background:var(--lime);display:grid;place-items:center;cursor:pointer;transition:background-color .2s}.send:hover:not(:disabled){background:var(--lime-hover)}.send:disabled{opacity:.45;cursor:default}.send svg{width:19px}
    ...
    @media(max-width:768px){.panel{inset:0;width:100vw;height:100dvh;max-height:none;border:0;transform:scale(.08);transform-origin:calc(100% - 45px) calc(100% - 45px)}.panel.open{transform:scale(1);border-radius:0}.fab{right:15px;bottom:15px}.coach{right:14px;bottom:88px;width:calc(100vw - 28px)}header{padding-top:max(18px,env(safe-area-inset-top))}}
  ...
  function reset(){
    ...
    renderMessage("bot","Здравствуйте! Я помогу с приложением, просмотром каналов, подписками и настройками Lime HD TV.");
  }
