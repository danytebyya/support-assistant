(function (global) {
  ...
  function init(config){
    ...
    root.innerHTML=`<style>.panel{position:fixed;right:22px;bottom:22px;width:min(390px,calc(100vw - 24px));height:min(650px,calc(100vh - 44px));opacity:0;visibility:hidden;pointer-events:none;transform:scale(.12);transform-origin:calc(100% - 30px) calc(100% - 30px);transition:transform .44s cubic-bezier(.22,.8,.25,1),opacity .14s ease,visibility 0s linear .44s;z-index:2147483646}.fab{position:fixed;right:22px;bottom:22px;width:60px;height:60px;border:0;border-radius:.75rem;background:#00871e;color:#fff;cursor:pointer;display:grid;place-items:center;opacity:1;visibility:visible;pointer-events:auto;z-index:2147483647;transition:background-color .2s,opacity .16s ease,transform .24s ease}.fab img,.fab svg{width:24px;height:24px;display:block;object-fit:contain;fill:none;stroke:currentColor;stroke-width:2}.panel.open ~ .fab{opacity:0;pointer-events:none;transform:scale(.72)}@media (max-width:768px){.fab{right:max(15px,env(safe-area-inset-right));bottom:max(15px,env(safe-area-inset-bottom))}}</style><link rel="stylesheet" href="${esc(cssUrl)}"><section class="panel" id="lime-ai-panel" role="dialog" aria-label="Чат поддержки" aria-hidden="true"><header><div class="logo">${iconMarkup("logo")||"L"}</div><div class="title"><b>${esc(state.config.title)}</b><span>Поддержка Lime HD TV</span></div><button class="header-action new-chat" type="button" aria-label="Новый чат" data-tooltip="Новый чат">${iconMarkup("edit")}</button><button class="header-action x" type="button" aria-label="Закрыть чат" data-tooltip="Закрыть чат">${iconMarkup("close")}</button></header><main class="messages" aria-live="polite"></main><form><textarea maxlength="1000" rows="1" placeholder="Напишите вопрос…" aria-label="Сообщение"></textarea><button class="send" aria-label="Отправить">${iconMarkup("send")}</button></form><div class="note">Ответы формируются на основе базы знаний <a href="https://limehd.tv/" target="_blank" rel="noopener">Lime HD TV</a></div></section><button class="fab" aria-label="Открыть чат" aria-controls="lime-ai-panel" aria-expanded="false">${iconMarkup("chat")}</button>`;
